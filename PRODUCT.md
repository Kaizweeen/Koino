...
moment to linger, arranged as a single calm arc a person can complete in a few
...
Koino meets you in a mood, not a checklist. Three things are core and must be preserved together
...
The combination (a devotion that meets you in an emotional theme, keeps it short and finishable,
and leaves you something to keep) is the distinctive position.
...
and track their streak. One devotion is surfaced per calendar day.

## Capabilities and Constraints

- Twelve rotating emotional themes, each with its own curated verses
  (`src/lib/themes.ts`, `src/lib/devotions/content.ts`).
...
- Real curated devotion content exists in `src/lib/devotions/content.ts` (WEB verses,
  reflections, prayers) and twelve real themes in `src/lib/themes.ts`.
